...
from utils.excel_timer_helper import (
    update_timer_label,
...
# Масштабирование расчётного напряжения
GAIN_SCALE = 10.0
...
class Sub234Window(Lab5SubBase):
    ...
    def __init__(self, controller, parent=None):
        ...
        self.setWindowTitle(
            "2.3.4 — Передаточная характеристика ИУ (пост. ток)"
        )
        ...
        self.r4_spin.valueChanged.connect(
            self._safe_recalculate
        )
        ...
        btn_graph = QPushButton(
            "График передаточной характеристики ИУ"
        )
        ...
        layout.addWidget(
            QLabel(
                "<b>2.3.4.</b> "
                "Передаточная характеристика ИУ "
                "(постоянный ток)<br>"
                "<small>"
                "K<sub>u.теор</sub> = −R4/R1, "
                "R1 = 10 кОм"
                "</small>"
            )
        )
    ...
    def _plot(self):
        ...
        uout_theor_arr = kt * uin_arr * GAIN_SCALE
        ...
        plt.title(
            f"Передаточная характеристика ИУ "
            f"(R4 = {r4} кОм)"
        )
    ...
    def _set_current_uout(self, value_v):
        ...
        self.table.setItem(
            row,
            COL_UOUT_E,
            self._editable_item(f"{value_v:.4f}")
        )
